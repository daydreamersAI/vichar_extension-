...
function handleMessage(message, sender, sendResponse) {
  try {
    ...
    // Add these payment-related handlers to the handleMessage function in background.js

// Find the handleMessage function and add these cases inside it
  else if (message.action === 'create_payment_order') {
    console.log("Processing create payment order request");
    
    const { amount, currency, packageId, credits, receipt } = message;
    ...
    .then(orderData => {
      sendResponse({ success: true, order: orderData });
    })
    .catch(error => {
  ...
  else if (message.action === 'verify_payment') {
    ...
    .then(verifyData => {
      // Update user data with new credits count
      if (verifyData.verified && verifyData.updatedCredits) {
        if (authState.user) {
          authState.user.credits = verifyData.updatedCredits;
          
          // Update storage
          chrome.storage.local.get(['userData'], (data) => {
            if (data.userData) {
              data.userData.credits = verifyData.updatedCredits;
              chrome.storage.local.set({ 'userData': data.userData });
            }
          });
        }
      }
      ...
      sendResponse({ 
        ...
        updatedCredits: verifyData.updatedCredits
      });
    })
    .catch(error => {
