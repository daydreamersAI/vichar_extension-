...
function fetchUserCredits() {
  ...
  creditsValueElement.textContent = localStorage.getItem('user_credits') || 'Loading...';

  fetch(`${API_URL}/credits/balance`, { headers: { 'Authorization': `Bearer ${token}` } })
    .then(response => {
        if (!response.ok) {
             // Attempt to parse error JSON from backend for better messages
             return response.json().then(err => { throw new Error(err.detail || `HTTP ${response.status}`) }).catch(() => { throw new Error(`HTTP ${response.status}`) });
        }
        return response.json();
    })
    .then(data => {
      if (typeof data.balance === 'number') {
          console.log("Credits received:", data.balance);
    ...
    .catch(error => {
      ...
      if (error.message.includes("401")) { // Unauthorized
          creditsValueElement.textContent = "N/A (Login?)";
          console.warn("Unauthorized fetching credits. Token might be invalid.");
          // Optionally trigger logout or login prompt here
      }
    });
}
...
function showCreditPurchaseOptions() {
     alert("Please use the extension popup icon (next to the address bar) to purchase credits.");
     // For a more integrated feel, you could message the background script
     // to programmatically open the popup, but this is simpler.
     // chrome.runtime.sendMessage({ action: "openExtensionPopup" }); // Need to implement this in background
 }
...
function captureCurrentPosition() {
  ...
  chrome.runtime.sendMessage({ action: "captureBoardForSidebar" }, (response) => {
    ...
    // Background script handles updating storage. Message listener below
    // ('updateSidebarImage') will trigger reloading data into the sidebar UI.
    if (chrome.runtime.lastError) {
      ...
      // Success! loadStoredBoardData will be called by the 'updateSidebarImage' message handler
    }
  });
}
