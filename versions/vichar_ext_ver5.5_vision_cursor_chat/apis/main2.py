...
@app.post("/analysis-with-credit")
async def analyze_with_credit(
    request: ChessAnalysisRequest,
    current_user = Depends(get_current_user)
):
    """Analyze a chess position with credit deduction"""
    if not current_user:
        raise HTTPException(
            status_code=401,
            detail="Authentication required"
    ...
    # Define credit cost - can be adjusted based on complexity
    credit_cost = 10
    
    # First deduct credits
    try:
        credit_response = await use_credits(
            {"amount": credit_cost},
            current_user
        )
        
        if not credit_response.get("success", False):
            raise HTTPException(
                status_code=400,
                detail="Failed to deduct credits"
            )
            
        # Proceed with analysis now that credits are deducted
        result = await analyze_position(request)
        
        # Add credit information to the response
        result["credits"] = {
            "used": credit_cost,
            "remaining": credit_response.get("current_balance", 0)
        }
        
        return result
        
    except HTTPException as e:
        if e.status_code == 400 and "Insufficient credits" in e.detail:
            # Special case for insufficient credits
            raise HTTPException(
