  ...
  if (request.action === "analyzeChessPosition") {
    ...
    (async () => {
      try {
        ...
        const storageData = await chrome.storage.local.get(['auth_token']);
        const token = storageData.auth_token;
        if (!token) {
          sendResponse({
            success: false,
            error: "Authentication required. Please log in to use the analysis feature.",
            errorCode: "AUTH_REQUIRED"
          });
          return;
        }
        
        console.log("Calling credit-based API for analysis");
        
        // Call the API with credits
        const response = await fetch("https://api.beekayprecision.com/analysis-with-credit", {
          method: 'POST',
          headers: {
        ...
        sendResponse({
          success: true,
          data: data.response,
          credits: data.credits || { used: 1, remaining: 0 }
        });
